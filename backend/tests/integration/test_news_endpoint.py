...
def test_search_news(mocker):
    ...
    mock_get_new_info = mocker.patch("src.posts.routers.get_new_info", return_value=[
...
def test_news_summary(mocker, test_token):
    ...
    request_body = NewsSumaryRequestSchemaWithModel(content="Test news content",ai_model="openai")
...
def test_news_summary_with_custom_model(mocker, test_token):
    headers = {"Authorization": f"Bearer {test_token}"}
    openai_response = json.dumps({"影響": "test impact generated by mocked openai", "原因": "test reason generated by mocked openai"})
    mock_openai(mocker, openai_response)

    request_body = NewsSumaryRequestSchemaWithModel(content="Test news content", ai_model="openai")
    response = client.post("/api/v1/news/news_summary_with_custom_model", json=request_body.dict(), headers=headers)

    assert response.status_code == 200
    json_response = response.json()
    assert json_response["summary"] == "test impact generated by mocked openai"
    assert json_response["reason"] == "test reason generated by mocked openai"

    anthropic_response = json.dumps(
        {"影響": "test impact generated by mocked anthropic", "原因": "test reason generated by mocked anthropic"})
    mock_anthropic(mocker, anthropic_response)
    request_body = NewsSumaryRequestSchemaWithModel(content="Test news content", ai_model="anthropic")
    response = client.post("/api/v1/news/news_summary_with_custom_model", json=request_body.dict(), headers=headers)
    assert response.status_code == 200
    json_response = response.json()
    assert json_response["summary"] == "test impact generated by mocked anthropic"
    assert json_response["reason"] == "test reason generated by mocked anthropic"